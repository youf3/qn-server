...
def mysql_ping_listener(dbapi_conn, connection_rec, connection_proxy):
    ...
    try:
        ...
        if ex.args[0] in (2006, 2013, 2014, 2045, 2055):
            msg = f"Got mysql server has gone away: {ex}"
...
def psql_convert_decimal_to_float(dbapi_conn, connection_rec):
    ...
    try:
        ...
        raise QuantnetException("Trying to use PostgreSQL without psycopg2 or psycopg2-binary installed!")
...
def _get_engine_poolclass(poolclass):
    ...
    if poolclass not in SQLA_CONFIG_POOLCLASS_MAPPING:
        raise Exception(f"Unknown poolclass: {poolclass}")
...
def retry_if_db_connection_error(exception):
    ...
    if isinstance(exception, (OperationalError, Exception)):
        conn_err_codes = (
            "2002",
            "2003",
            "2006",  # MySQL
...
def read_session(function: "Callable[P, R]"):
    ...
    @retrying(retry_on_exception=retry_if_db_connection_error, wait_fixed=500, stop_max_attempt_number=2)
